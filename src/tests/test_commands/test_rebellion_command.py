...
"""
Tests for rebellion commander assignment — migrated to war_system.assign_rebellion_commanders()
"""
...
def test_assign_rebellion_commander_with_designate_governor(
    ...
    """测试行省有候任总督时，通过 war_system.assign_rebellion_commanders 指派其为指挥官"""
    ...
    # 通过新的 war_system 方法指派
    ws = state.get_war_system()
    results = ws.assign_rebellion_commanders()
    ...
    assert results[0]["rebellion_id"] == war.id
    assert results[0]["commander_id"] == designate.id
...
def test_rebellion_war_timeline(
    ...
    """完整流程测试：广场阶段触发起义 -> war_system.assign_rebellion_commanders -> 战斗阶段镇压"""
    state = setup_game_state
    faction = add_faction("test", "Test Faction")
    governor = add_figure(101, "Governor", faction.id)
    province = create_province_with_governor(1, governor.id, designate_id=None)
    ...
    # 通过新的 war_system 方法指派指挥官
    results = war_system.assign_rebellion_commanders()
    assert war.commander_id == governor.id
    assert governor.is_absent is True
    assert len(results) == 1
