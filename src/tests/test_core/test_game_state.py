...
def test_conquer_provinces_preserves_governor_type():
    """验证征服行省后 governor_type 保持不变"""
    state = GameState.create_for_testing({})
    province = Province(
        province_id=1,
        name="西西里",
    ...
    war._unlocked_provinces = [1]
    war_system = Mock()
    war_system.get_war_by_id.return_value = war
    state.get_war_system = Mock(return_value=war_system)

    state.conquer_provinces("test_war")

    # 重新获取行省
    conquered_province = state.get_province(1)
    assert conquered_province.conquered is True
    assert conquered_province.governor_type == "proconsul"  # 保持不变
