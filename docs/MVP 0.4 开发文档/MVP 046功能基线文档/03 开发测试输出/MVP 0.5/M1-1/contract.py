...
class Contract:
    ...
    # === MVP 0.5 新增字段 ===
    _profit_base: int = 0  # 基础利润
    _is_under_execution: bool = False  # 是否执行中
    _complete_turn: Optional[int] = None  # 完成回合
    ...
    def mark_winner(self, winner_id: int, current_turn: int, profit_base: int) -> None:
        ...
        Args:
            winner_id: 中标人物的ID
            current_turn: 中标时的当前回合数
            profit_base: 合同的基础利润

        Raises:
            ValueError: 如果合同已经处于激活状态
        """
        if self.status != ContractStatus.PENDING:
